...
node('maven') {
  ...
  stage('Build war') {
    def mvnCmd = "mvn -s ./nexus_openshift_settings.xml package -DskipTests"
  }

  // Using Maven run the unit tests
  stage('Unit Tests') {
    def mvnCmd = "mvn -s ./nexus_openshift_settings.xml test"
  }
  ...
  stage('Publish to Nexus') {
    def mvnCmd = "mvn -s ./nexus_openshift_settings.xml deploy -DskipTests -DaltRepository TestingCandidate-${version}'
  }
