...
node('maven') {
  ...
  stage('Publish to Nexus') {
    sh "$mvnCmd release:clean release:prepare release:perform -DreleaseVersion=${releaseVersion} -DdevelopmentVersion=${developmentVersion}"
    sh "$mvnCmd deploy -DskipTests -DaltRepository http://nexus3-nho-nexus.apps.248d.openshift.opentlc.com/repository/releases"
  }
